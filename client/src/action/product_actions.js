...
export const saveProduct = (dataToSubmit, id) => async dispatch => {
    const { data } = await axios.post(`${PRODUCT_SERVER}/updateArticle?id=${id}`, dataToSubmit);
    if(data.success){
...
export const removeArticle = (id, previousData) => async dispatch => {
    ...
    const {data} = await axios.delete(`${PRODUCT_SERVER}/remove_article?id=${id}`);

    if(data.success){
        let index = previousData.findIndex(item => item._id === id);
        let newArticles = [...previousData.slice(0, index), ...previousData.slice(index + 1)];
