...
class AddProduct extends Component {
  ...
  deleteAtribute = (id, type) => {
    const newFormData = { ...this.state.formData };
    let index = newFormData[type].addedValues.findIndex(
      item => item._id === id
    );
    let newArray = [
      ...newFormData[type].addedValues.slice(0, index),
      ...newFormData[type].addedValues.slice(index + 1)
    ];
